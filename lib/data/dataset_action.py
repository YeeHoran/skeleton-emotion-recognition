...
def normalize(motion):
    min_val = motion.min()
    max_val = motion.max()
    normalized_motion = (motion - min_val) / (max_val - min_val + 1e-8)
...
class ActionDataset(Dataset):
    def __init__(self, data_path, data_split, n_frames=240, random_move=True, scale_range=[1,1], check_split=True):   # data_split: train/test etc.
        np.random.seed(0)
        ...
        for sample in annotations:
            ...
            motion = sample['keypoint'][:, resample_id]  # shape: (M, T, V, 3) → 不需要 make_cam
            motion = normalize(motion)
            #motion_conf = sample['keypoint_score'][..., None]
            #motion = np.concatenate((motion[:,resample_id], motion_conf[:,resample_id]), axis=-1)
            '''
